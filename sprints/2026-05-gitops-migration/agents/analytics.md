# Agent Analytics — fiche de mission sprint GitOps
...
|---|---|
| App | **Analytics** |
| Worktree | `~/Bureau/veridian-platform-analytics/` |
| Branche cible | `feat/analytics-gitops-migration` |
| Stack Dokploy slug | `compose-synthesize-virtual-transmitter-i9bv43` |
| Compose path cible repo | `infra/services/analytics/docker-compose.yml` |
| Domaine prod | `analytics.app.veridian.site` |
| Image app | `ghcr.io/christ-roy/analytics:latest` |
| Image DB | (pas de DB séparée) |
| Network Docker | `compose-synthesize-virtual-transmitter-i9bv43_default` |

## Pièges / notes spécifiques

**Note** : tag `:latest` en prod (anti-pattern à fixer pendant le SHA-pinning). Pas de DB séparée — utilise core DB Veridian.

## Endpoints à smoke test

```bash
curl -sI https://analytics.app.veridian.site
...
Suis [gitops-pattern.md](../gitops-pattern.md) en remplaçant `<APP>` par `analytics`.

Procédure résumée :

1. **Snapshot** : `tmp/dokploy-snapshot-analytics-$(date +%Y%m%d-%H%M)/` dans ton worktree
   ```bash
   mkdir -p tmp/dokploy-snapshot-analytics-$(date +%Y%m%d-%H%M)
   ssh prod-pub 'sudo cat /etc/dokploy/compose/compose-synthesize-virtual-transmitter-i9bv43/code/docker-compose.yml' \
     > tmp/dokploy-snapshot-analytics-*/docker-compose-live.yml
   ssh prod-pub 'docker inspect $(docker ps -q -f name=analytics-prod)' \
     > tmp/dokploy-snapshot-analytics-*/containers-inspect.json
   ssh prod-pub 'docker exec dokploy-traefik wget -qO- http://localhost:8080/api/http/routers 2>/dev/null' | \
     jq '.[] | select(.service | contains("analytics"))' \
     > tmp/dokploy-snapshot-analytics-*/traefik-routers.json
   ```

2. **Créer `infra/services/analytics/`** :
...
3. **Pinning SHA** :
   ```bash
   ssh prod-pub 'docker images --digests | grep -E "ghcr.io/christ-roy/analytics"'
   ```
...
Adapter à analytics (nom image, paths).

## Phase C — Loop validation 7 jours

Quotidien :

1. `obs check security` → 0 CRIT/HIGH sur image analytics deployed
...
- [ ] Analytics en mode Git Dokploy avec auto-deploy webhook
- [ ] Compose `infra/services/analytics/docker-compose.yml` images SHA-pinned
...
- **Si analytics.app.veridian.site casse** → restore Raw immédiatement
