# Agent Verger Shop — fiche de mission sprint GitOps
...
| Branche cible | `feat/verger-shop-gitops-migration` |
| Stack Dokploy slug | `verger-shop-ozjjew` |
| Compose path cible repo | `infra/services/verger-shop/docker-compose.yml` |
| Domaine prod | `les-vergers-de-faverolles.fr` |
| Image app | `ghcr.io/christ-roy/verger-faverolles-shop:latest` |
| Image DB | `postgres:16-alpine` |
| Network Docker | `verger-shop-ozjjew_default` |

## Pièges / notes spécifiques

**ATTENTION INCIDENT** : compromis 2026-05-07 (XMRig mineur via CVE Next.js 15.1.6 + auth bypass middleware). Tous les secrets ont été rotés. **Audit Trivy obligatoire avant CI auto-merge** car l'app a déjà été compromise une fois. Volume DB : `verger-shop-ozjjew_postgres_data`.

## Endpoints à smoke test

```bash
curl -sI https://les-vergers-de-faverolles.fr
...
Suis [gitops-pattern.md](../gitops-pattern.md) en remplaçant `<APP>` par `verger-shop`.

Procédure résumée :

1. **Snapshot** : `tmp/dokploy-snapshot-verger-shop-$(date +%Y%m%d-%H%M)/` dans ton worktree
   ```bash
   mkdir -p tmp/dokploy-snapshot-verger-shop-$(date +%Y%m%d-%H%M)
   ssh prod-pub 'sudo cat /etc/dokploy/compose/verger-shop-ozjjew/code/docker-compose.yml' \
     > tmp/dokploy-snapshot-verger-shop-*/docker-compose-live.yml
   ssh prod-pub 'docker inspect $(docker ps -q -f name=verger-shop-prod)' \
     > tmp/dokploy-snapshot-verger-shop-*/containers-inspect.json
   ssh prod-pub 'docker exec dokploy-traefik wget -qO- http://localhost:8080/api/http/routers 2>/dev/null' | \
     jq '.[] | select(.service | contains("verger-shop"))' \
     > tmp/dokploy-snapshot-verger-shop-*/traefik-routers.json
   ```

2. **Créer `infra/services/verger-shop/`** :
...
3. **Pinning SHA** :
   ```bash
   ssh prod-pub 'docker images --digests | grep -E "ghcr.io/christ-roy/verger-faverolles-shop"'
   ```
...
Adapter à verger-shop (nom image, paths).

## Phase C — Loop validation 7 jours

Quotidien :

1. `obs check security` → 0 CRIT/HIGH sur image verger-shop deployed
...
- [ ] Verger Shop en mode Git Dokploy avec auto-deploy webhook
- [ ] Compose `infra/services/verger-shop/docker-compose.yml` images SHA-pinned
...
- **Si les-vergers-de-faverolles.fr casse** → restore Raw immédiatement
