...
### P0.2 — ✅ RÉSOLU (audit 2026-05-13) : Observabilité Grafana Cloud déjà en place

**Statut** : ✅ infra obs branchée et opérationnelle. Audit du 2026-05-13 confirme :
- `obs health` : Loki OK, 1M logs/h dev + 200k/h prod, Prom series actifs
- Alloy v1.16.1 actif sur prod (`/etc/alloy/config.alloy`) + dev — IaC dans `grafana/alloy/config.alloy.tmpl`
- Healthcheck externe : `veridian-prod-healthcheck.service` actif sur dev → alerte Telegram
- Container monitoring : `veridian-docker-monitor.service` + `veridian-system-monitor.service` actifs prod et dev
- CLI `obs` (Python+Typer+Rich) installé : `obs check`, `obs tail`, `obs pentest`, etc.
- Stack URL : https://brunon5robert.grafana.net (Grafana Cloud free 50GB/mois, 14j retention)
- Doc complète : `grafana/README.md` (architecture, layout, install)

**Reste à faire (nice-to-have, pas urgent)** :
- [ ] Dashboards Grafana par app (latence, erreurs, throughput) — pour l'instant on lit via `obs check apps`
- [ ] `runbooks/monitoring-access.md` (synthèse 1 page : URLs, comptes Grafana, comment lire les dashboards) — partiellement couvert par `grafana/README.md`

**Vraie urgence** : pas de bouton "Loki retention 14j" — si on doit aller au-delà pour forensique, on a une fenêtre limitée. À surveiller.
...
- [x] Doomsday consécutifs validés : 5 runs après le fix (1001+1101+1001+1001+1001 ≈ 5105 req), tous ~100% 200 OK, 0% 403, latence avg ~1.6s.
- [x] **Couverture DDoS améliorée** (audit Robert 2026-05-13 11:08) : collection `crowdsecurity/http-dos` installée (4 scenarios : `bypass-cache`, `random-uri`, `switching-ua`, `invalid-http-versions`). IaC : `infra/crowdsec/compose.yml` updated avec la nouvelle collection dans `COLLECTIONS`. **DDoS volumetric mono-URL mono-UA pas couvert** par CrowdSec — draft `infra/traefik/dynamic/rate-limit.yml` créé mais en mode **fusible large** (avg 100 req/s, burst 300, pas de ban persistant, juste 429 Traefik). Philosophy : pas pixel-perfect, juste éviter qu'un botnet fasse tomber la prod. Une app qui bug en boucle côté client (~20 req/s max) passe sous le seuil sans gêne. Pas activé par défaut, à brancher manuellement dans `entryPoints.websecure.http.middlewares` quand validé.
...
### P1.5 — ✅ RÉSOLU 2026-05-13 : Runbooks incidents typiques (4/4 écrits)

- [x] `runbooks/incidents/db-down.md` — basé sur incident 2026-05-08 prospection
- [x] `runbooks/incidents/traefik-collision.md` — basé sur incident 2026-05-10 (dual-router hub) + skill `06-blue-green-procedure.md`
- [x] `runbooks/incidents/container-OOM.md` — patterns OOM kill + limits Docker
- [x] `runbooks/incidents/cve-active-prod.md` — procédure quand une CVE high apparaît (basé sur verger-shop 2026-05-07)
- [ ] **NEW** `runbooks/incidents/2026-05-13-bouncer-fix.md` (cette session) — postmortem migration plugin Traefik
